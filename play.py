            ...
            for a1 in self.inc_action_space:
                for a2 in self.inc_action_space:
                    if (prev_action[0] + a1) == 0 and (prev_action[1] + a2) == 0:
                        continue
                    elif abs(prev_action[0] + a1) >= 5 or (prev_action[1] + a2) >= 5:
                        continue
                    else:
                        if greedy_action is None:
                            greedy_action = self.action_value_function[
                                prev_state, (prev_action[0] + a1, prev_action[1] + a2)]
                            continue
                        if greedy_action < self.action_value_function[
                            prev_state, (prev_action[0] + a1, prev_action[1] + a2)]:
                            tup = (a1, a2)
                    if tup is None:
